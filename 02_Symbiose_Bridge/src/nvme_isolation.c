...
SymbioseIsolateNvmeDevice(_In_ WDFDEVICE Device, _In_ ULONG VendorId,
  ...
  if (!NT_SUCCESS(status)) {
    WdfWaitLockRelease(devCtx->StateLock);
    KdPrint((
        "SymbioseBridge: Failed to find PCI device VID=%04X DID=%04X: 0x%08X\n",
  ...
  if (!NT_SUCCESS(status)) {
    ...
    WdfWaitLockRelease(devCtx->StateLock);
    KdPrint(
        ("SymbioseBridge: Failed to detach driver stack: 0x%08X\n", status));
    return status;
  }

  // Step 3: Load SymbioseNull.inf as the function driver
  // This creates an "airlock" - Windows sees the device but cannot access its
  // storage
  ...
  KdPrint(
      ("SymbioseBridge: NVMe device VID=%04X DID=%04X isolated successfully\n",
       VendorId, DeviceId));
...
NTSTATUS
SymbioseRestoreNvmeDevice(_In_ WDFDEVICE Device, _In_ ULONG DeviceIndex) {
  NTSTATUS status;
  PSYMBIOSE_DEVICE_CONTEXT devCtx = NULL;
  ...
  status = SymbioseUnloadNullDriver(
      &devCtx->IsolatedDevices[DeviceIndex].DevicePath);
  if (!NT_SUCCESS(status)) {
    WdfWaitLockRelease(devCtx->StateLock);
  ...
  status = SymbioseReattachDriverStack(
      &devCtx->IsolatedDevices[DeviceIndex].DevicePath);
  if (!NT_SUCCESS(status)) {
    WdfWaitLockRelease(devCtx->StateLock);
    KdPrint(
        ("SymbioseBridge: Failed to re-attach driver stack: 0x%08X\n", status));
    return status;
  }
