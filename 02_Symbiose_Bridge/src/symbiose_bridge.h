...
#define MAKE_ULONGLONG(a, b, c) (((ULONGLONG)(a) << 32) | ((ULONGLONG)(b) << 16) | (c))
#define TRUE 1
#define FALSE 0
...
#define WDF_DRIVER_CONFIG_INIT(config, initFunc) do { (config)->EvtDriverDeviceAdd = initFunc; } while(0)
#define WdfDriverCreate(DriverObject, RegistryPath, Attributes, Config, Handle) STATUS_SUCCESS
...
#define KeSetEvent(Event, Increment, Wait) do {} while(0)
#define WDF_REL_TIMEOUT_IN_MS(ms) (-(long long)((ms) * 10000))
#define KeSetTimer(Timer, DueTime, Dpc) do {} while(0)
#define WdfWaitLockAcquire(Lock, Timeout) do {} while(0)
#define WdfWaitLockRelease(Lock) do {} while(0)
#define RtlFreeUnicodeString(Str) do {} while(0)
...
#define WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(type, name) \
    type* name(WDFDEVICE device) { static type ctx; return &ctx; }
#endif
...
void SymbioseIoctlHandler(void* Queue, void* Request, size_t OutputBufferLength, size_t InputBufferLength, ULONG IoControlCode);

NTSTATUS SymbioseIsolateNvmeDevice(_In_ WDFDEVICE Device, _In_ ULONG VendorId, _In_ ULONG DeviceId);
NTSTATUS SymbioseRestoreNvmeDevice(_In_ WDFDEVICE Device, _In_ ULONG DeviceIndex);
NTSTATUS SwitchToChaosKernel(_In_ PVOID KernelImage, _In_ SIZE_T KernelSize, _In_ PVOID RamdiskImage, _In_ SIZE_T RamdiskSize, _In_ PVOID BootParams);
NTSTATUS SymbioseInjectInitParameter(_In_ PVOID BootParams, _In_ const char* InitPath);
