...
HANDLE CreateFileW(const void* lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, void* lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) { return INVALID_HANDLE_VALUE; }
int GetFileSizeEx(HANDLE hFile, LARGE_INTEGER *lpFileSize) { return 0; }
void* VirtualAlloc(void* lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect) { return NULL; }
int ReadFile(HANDLE hFile, void* lpBuffer, DWORD nNumberOfBytesToRead, DWORD* lpNumberOfBytesRead, void* lpOverlapped) { return 0; }
void VirtualFree(void* lpAddress, SIZE_T dwSize, DWORD dwFreeType) {}
void CloseHandle(HANDLE hObject) {}
DWORD GetLastError() { return 0; }
int ExpandEnvironmentStringsW(const void* lpSrc, WCHAR* lpDst, DWORD nSize) { return 0; }
int DeviceIoControl(HANDLE hDevice, DWORD dwIoControlCode, void* lpInBuffer, DWORD nInBufferSize, void* lpOutBuffer, DWORD nOutBufferSize, DWORD* lpBytesReturned, void* lpOverlapped) { return 0; }
...
#endif
{
	...
	status = LoadFileIntoBuffer((const void*)kernelPath, &kernelBuffer, &kernelSize);
	...
	status = LoadFileIntoBuffer((const void*)ramdiskPath, &ramdiskBuffer, &ramdiskSize);
	if (!NT_SUCCESS(status)) {
