...
int SymbioseIrcd_RecvJumbo(PSYMBIOSE_CLIENT client, SYMBIOSE_MSG_TYPE *type,
  ...
  if (client->jumbo_buffer == NULL) {
    ...
    if (!client->jumbo_buffer) return -1;
  } else if (client->jumbo_buffer_size < header.payload_len) {
    uint8_t *new_buf = (uint8_t *)realloc(client->jumbo_buffer, header.payload_len);
    if (!new_buf) return -1;
  ...
  // memcpy(client->jumbo_buffer + header.chunk_offset, chunk_data, header.chunk_len);
